...
}: DropdownComponentPropsType) => {
  const theme = useTheme();
  const rowValue = row.original[column.id] || '';
  const [isInitValue, setIsInitValue] = useState<boolean>(true);
  const [value, setValue] = useState<string>(rowValue);
