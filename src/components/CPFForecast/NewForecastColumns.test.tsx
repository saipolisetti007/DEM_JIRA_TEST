...
describe('NewForecastColumns', () => {
  ...
  test('handles local change correctly', () => {
    ...
    const columns = result.current;
    const editUnitColumn: any = columns.find((col) => col.accessorKey === 'editedUnits');
    ...
    const editUnitColumnProps = result.current
      ?.find((col) => col.accessorKey === 'editedUnits')
      ?.muiEditTextFieldProps?.({ row, column });

    const cellValue = editUnitColumnProps ? editUnitColumnProps.value : undefined;
