...
describe('CPFForecastMain', () => {
  beforeEach(() => {
    (cpfFilters as jest.Mock).mockResolvedValue(mockFilters);
    (cpfGetForecast as jest.Mock).mockResolvedValue(mockData);
    (cpfSkuForecast as jest.Mock).mockResolvedValue(cpfData);
    (useParams as jest.Mock).mockReturnValue({ status: 'Pending approval' });
  });
  ...
  test('should handle filter fetch error', async () => {
    (cpfFilters as jest.Mock).mockRejectedValueOnce(new Error('Network Error'));
    await act(async () =>
      render(
        <Provider store={store}>
          <MemoryRouter>
  ...
  test('should handle fetch error', async () => {
    (cpfGetForecast as jest.Mock).mockRejectedValueOnce(new Error('Network Error'));
    await act(async () =>
      render(
        <Provider store={store}>
          <MemoryRouter>
