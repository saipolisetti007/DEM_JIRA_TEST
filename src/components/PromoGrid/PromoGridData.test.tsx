...
describe('PromoGridData Component', () => {
  beforeEach(() => {
    (promoGridFilters as jest.Mock).mockResolvedValue(mockFilters);
  ...
  test('should filter fetch error', async () => {
    (promoGridFilters as jest.Mock).mockRejectedValueOnce(Error);
    await act(async () =>
      render(
        <Provider store={store}>
          <BrowserRouter>
  ...
  test('cancels row successfully', async () => {
    (cancelRowData as jest.Mock).mockResolvedValueOnce({});
  ...
  test('handles Excel file upload', async () => {
    ...
    (uploadDataExcel as jest.Mock).mockResolvedValueOnce({ data: {} });
    await act(async () => {
      render(
        <Provider store={store}>
          <BrowserRouter>
  ...
  test('handles upload failure', async () => {
    ...
    (uploadDataExcel as jest.Mock).mockRejectedValueOnce(new Error('Upload failed'));
    await act(async () => {
      render(
        <Provider store={store}>
          <BrowserRouter>
  ...
  test('handles upload failure based on golden customer id', async () => {
    ...
    (uploadDataExcel as jest.Mock).mockRejectedValueOnce(errorResponse);
    await act(async () => {
      render(
        <Provider store={store}>
          <BrowserRouter>
  ...
  test('handles fail data Excel file download', async () => {
    (downloadDataExcel as jest.Mock).mockRejectedValueOnce(
  ...
  test('should handle data loading cancellation', async () => {
    ...
    await act(async () => {
      (uploadDataExcel as jest.Mock).mockImplementationOnce(() => {
        throw new Error('AbortError');
      });
    });

    expect((screen.getByTestId('upload') as HTMLInputElement).value).toBe('');
  });
